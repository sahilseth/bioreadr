...
create_casava_sample_sheet <- function(runid,intPath=get_opts("ngs.intPath")){
    ass <- get_opts("ngs.NGSAssignment")
    if(is.null(ass)){options(ngs.NGSAssignment=getAssignment(runid));ass <- get_opts("ngs.NGSAssignment")}
    mask <- get_opts("ngs.mask");indLen <- gsub(".*I([0-9]*).*","\\1",mask)#index,length
...
## sets up seqLen also
get_casava_mask <- function(path=get_opts("ngs.bclPath")){
    require(XML)
    ##ass <- get_opts("ngs.NGSAssignment")
