...
annotate_variants <- function(x, 
  ...
  # if outfile is missing, this becomes, current WD
  outpath = dirname(outfile)
  outprefix = tools::file_path_sans_ext(basename(outfile))
  ...
  input = sprintf("%s/%s.temp", outpath, outprefix)
  out_ann = sprintf("%s.%s_multianno.txt", outprefix, build)
  ...
  ann = tbl_df(read_sheet(out_ann, quote = ""))
  #ann = fread(outfile)
  #print(head(ann))
  #print(head(mutMat))
  ...
  message("Writing out final output: ", outfile)
  write.table(ann2, file = outfile, sep = "\t", quote = FALSE, row.names = FALSE)
  warnings()
  
  # remove temp files
  unlink(c(out_ann, input))
  
  invisible(list(cmd = cmd, outfile = outfile, data = ann2))
}
